<template>
	<view class="u-search" @tap="clickHandler" :style="{
		...
		<view class="u-content" :style="{ backgroundColor: bgColor, borderRadius: shape == 'round' ? '100rpx' : '10rpx',
				border: borderStyle, height: height + 'rpx' }">
			...
			<input confirm-type="search" @blur="blur" :value="value" @confirm="search" @input="inputChange"
				...
				placeholder-class="u-placeholder-class" :placeholder="placeholder"
				:placeholder-style="`color: ${placeholderColor}`" class="u-input" type="text" :style="[{
		...
		<view :style="[actionStyle]" class="u-action" :class="[showActionBtn || show ? 'u-action-active' : '']"
			@tap.stop.prevent="custom">{{ actionText }}</view>
	</view>
</template>
